...
さらに言えば、CLIをAIエージェントに組み込むと自然言語で分析できるようになります。

実際に僕がAIエージェントに「イラン・アメリカ戦争がいつ終わるか教えて」と聞いたところ、エージェントが裏で `polymarket markets search "Iran ceasefire"` を実行して、こんな分析を返してくれました。

> 2026年3月時点でのPolymarketの見立て：
>
> | 停戦期限 | 確率 |
> |---------|------|
> | 3/6まで | 11.5% |
...
Webアプリを作らなくても、CLIがあればAIエージェントが予測市場のアナリストになってくれます。
...
インストールはRustソースビルドが確実で、GLIBC問題にだけ注意が必要です。市場検索、オッズ、オーダーブック、価格履歴といったデータがコマンド一発で取得できて、テーブルとJSON形式を切り替えられるのでスクリプトとの連携も容易です。データ取得だけならウォレット認証は不要で、Node.jsと組み合わせればオッズ変動の監視も簡単に実装できます。
