...
例えば「米イラン戦争はいつ終わるのか？」をCLIで調べるとこうなります。

```bash
polymarket markets search "Iran ceasefire" --limit 5 -o json
```
...
ニュースでは「いつ終わるか」は曖昧ですが、予測市場は日付ごとに確率で示してくれます。CLIで取得したJSONをパースすれば、こうした分析がスクリプト1つで自動化できます。
