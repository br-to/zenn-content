...
## 実践: CLIとPythonでオッズ変動を監視する

CLIのJSON出力をPythonで処理すれば、オッズの変動監視が作れます。
...
CLI_PATH = "/usr/local/bin/polymarket"
HISTORY_FILE = Path("odds_history.json")
THRESHOLD = 5.0  # 5%以上の変動でアラート

WATCH_QUERIES = ["Bitcoin", "Ethereum", "Fed rate", "OpenAI", "AI"]

def search_markets(query, limit=3):
    """市場を検索"""
    r = subprocess.run(
        [CLI_PATH, "markets", "search", query,
...
for query in WATCH_QUERIES:
    for m in search_markets(query):
        ...
        # 前回との比較
        if mid in history:
            delta = yes_pct - history[mid]["yes_pct"]
            if abs(delta) >= THRESHOLD:
                direction = "📈" if delta > 0 else "📉"
                alerts.append(
                    f"{direction} {question}\n"
                    f"   {history[mid]['yes_pct']:.1f}% → {yes_pct:.1f}% "
                    f"({delta:+.1f}%)"
                )

        history[mid] = {"question": question, "yes_pct": round(yes_pct, 1)}
...
市場の検索、オッズの表示、オーダーブックの可視化...。ひと通り実装したのですが、CLIを触ってみたら全部コマンド一発でできてしまいました。

しかもJSON出力でスクリプトに渡せるので、Pythonと組み合わせれば監視も自動化できる。ブラウザを開いてポチポチ操作する必要がなくなりました。
...
- Pythonと組み合わせてオッズ変動の監視が簡単にできる

予測市場のオッズは「世界中のトレーダーによるリアルタイムの確率予測」です。賭けなくても、データソースとして十分な価値があります。

個人的には、このオッズ変動データが実際に株価やクリプト価格の先行指標になるのかを検証してみたいと思っています。引き続きキャッチアップしていきます。

記事が参考になった方は、ぜひZennのフォローや X（[@brto_0224](https://x.com/brto_0224)）のフォローもよろしくお願いします！🙌
