...
最近、OpenAIとStripeがAgentic Commerce Protocol（ACP）という仕様を発表し、ChatGPTの中で直接買い物ができるようになりました。
...
ACPはOpenAIとStripeが共同開発した仕様で、既存のカード決済インフラの上でエージェントが買い物する仕組みです。ChatGPTのInstant Checkout機能として実装されています。ただし現時点では米国限定です。
